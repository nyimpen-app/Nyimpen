	...
	private function __syncFacebookUser(){
		...
		if($Auth->user('id')){
			...
			elseif(empty($this->authUser) && $this->createUser) {
				...
				$this->authUser[$this->User->alias]['username'] = isset($this->me['email']) ? $this->me['username'] : $this->me['first_name'] . '_' . $this->me['last_name'];
				$this->authUser[$this->User->alias]['email'] = $this->me['email'];
				$this->authUser[$this->User->alias]['firstname'] = isset($this->me['first_name']) ? $this->me['first_name'] : NULL;
				$this->authUser[$this->User->alias]['lastname'] = isset($this->me['last_name']) ? $this->me['last_name'] : NULL;
				$this->authUser[$this->User->alias]['lastlogin'] = date('Y-m-d H:i:s');
				$this->authUser[$this->User->alias]['role_id'] = 1;
